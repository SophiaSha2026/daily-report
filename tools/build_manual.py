...
def main():
    ...
    E.append(P("<b>喂给影子/擂台的特征已扩到 27 个</b>，新增三组："
               "板块竞价一致性（同板块今天竞价里是不是一起动——来自 AI 归因"
               "发现的失败模式）、市场状态与交互项（「今天是什么日子」本身就是"
               "信息，模型借此学会看行情下菜）、昨日尾盘竞价占比（14:57-15:00 "
               "的收盘定价是机构行为，对次日早盘有信息，数据源 Tushare "
               "stk_auction_c，2025 年至今全量已回填）。", "b"))
    E.append(P("<b>AI 在调优流程里的四个通道</b>：① 每天归因（判定当天是"
               "板块轮动/大盘系统性/情绪退潮…，直接变成优化器里这一天的权重）"
               "② 参数提案（和优化器的提案走同一套闸门）③ 特征提案（进积压"
               "清单，人工实现——上面板块一致性就是第一个转正的）④ 审稿人"
               "（变更过完统计闸门后，Opus 拿完整证据包交叉检查「数字和叙事"
               "对不对得上」，意见附在变更邮件里；可配置成有搁置权，但只能"
               "按暂停不能按删除，你永远有终审权）。AI 永不直接排序、"
               "永不直接写参数值。", "b"))
