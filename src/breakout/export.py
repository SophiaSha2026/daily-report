...
DISCLAIMER = (
    ...
    f"{STREAK_PERF[4][1]}%（是随便买的 {STREAK_PERF[4][2]:.0f} 倍），"
    f"连续 2 天以上 {STREAK_PERF[3][1]}%，再往上样本太少不作数。"
    f"全市场随便买一只是 {BASE}%。"
...
def _body(date: str, a: pd.DataFrame, b: pd.DataFrame, meta: dict,
    ...
    tc = (f'<h1 style="margin-top:22px">连续天数怎么看</h1>'
          f'<div class="sub">下面是 {PERF["window"]}的实测准确率，口径和每天发的'
          f'清单完全一致（≥97 分按预测值取前 10，连续按上榜天数算，'
          f'训练集做了 20 日净化）。</div>'
          f'<table><tr><th>连续天数</th><th>涨超 50% 的比例</th>'
          f'<th>相对随便买</th><th>样本</th></tr>{rows}</table>'
          f'<div class="tip">连续 2 天以上（{STREAK_PERF[3][1]}%，'
          f'{STREAK_PERF[3][3]} 只）比全部上榜（{STREAK_PERF[4][1]}%）略高；'
          f'3 天以上各档样本只有 {STREAK_PERF[2][3]}、{STREAK_PERF[1][3]}、'
          f'{STREAK_PERF[0][3]} 只，数字上下抖动是样本少，不是信号。'
          f'带 🔥 的是连续 3 天以上，只是提示它已经在榜上待了几天。</div>')
