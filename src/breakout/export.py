...
# 给新规则背书。数字来源：docs/breakout_log.md 实验 8（验证集 207 个交易日）。
#
# 清单 A 的规则是「≥97 分才上榜」，而**连续够格的天数**是清单里最强的
# 单一信号，所以准确率按连续天数分档报，不报一个笼统的平均值。
#
# 实验 7 那组数字（32.0 / 25.8 / 15.4）是在「每天只看前 30 名」的口径下
# 算的，和生产不一致：生产是「≥97 分的按预测值取前 10」。实验 8 用生产
# 口径重算，首日那档从 15.4% 变成 18.0%，档位也从 3 天扩到 5 天
# （连续 4、5 天确实还在往上走，之前一律并进「3 天及以上」看不出来）。
